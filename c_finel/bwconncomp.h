...
void floodFill(int *BW, int rows, int cols, int x, int y, int componentId, int *labels, int *componentPixels, int *pixelCount) {
    ...
    while (top >= 0) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (nx >= 0 && nx < rows && ny >= 0 && ny < cols && BW[nx * M_S + ny] == 1 && labels[nx * M_S + ny] == 0) {
                top++;
                stack[top][0] = nx;
                stack[top][1] = ny;
            }
...
ConnectedComponents *bwconncomp(int *BW, int rows, int cols) {
    ...
    int *labels = (int *)malloc(M_S * N_S * sizeof(int *));
    ...
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            ...
            if (BW[i * M_S + j] == 1 && labels[i * M_S + j] == 0) {
                // Array to store pixel indices for the current component
                int *componentPixels = (int *)malloc((M_S * N_S + 1) * sizeof(int));
                int pixelCount = 0;
