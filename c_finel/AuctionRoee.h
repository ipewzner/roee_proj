...
// Utility function to find1 the index of the max value in an array
...
// Utility function to find1 indices of zero elements
void find1(int *array, int length, int *indices) {
    int index = 0;
    for (int i = 0; i < length; i++) {
        if (array[i] == -1) {
            indices[index++] = i;
...
void AuctionRoee(double *assign_mtrx, int N, int M, int *Assigned) {
    ...
    if (N > M) {
        while (sum(Assigned_indx, M) != M) {
            ...
            int indices[zz]; 
            find1(Assigned_indx, M, indices); // find1 unassigned plots
            ...
            for (int q = 0; q < zz; q++) {
                ...
                assignment_val[i1] = 100 * assignment_val[i1]; // find1 next best bid
        ...
        while (sum(Assigned_indx, N) != N) {
            ...
            int indices[zz];
            find1(Assigned_indx, N, indices); // find1 unassigned tracks
