...
#include "bwconncomp.h"  // Include the bwconncomp function

// Function prototype
/*
int main2() {
    // Example binary image (3x3)
    int BinaryMap[3][3] = {{1, 1, 0}, {0, 0, 0}, {1, 0, 0}};
    ...
    DetectionBlobsRoee(&BinaryMap[0][0], 3, 3, 1, 10, &ReportCL, &NumObjects);

    // Output detected blobs
    printf("\nDetected blobs: %d\n", NumObjects);
    for (int i = 0; i < NumObjects; i++) {
        printf("Blob %d mean row: %.2f\n", i + 1, ReportCL[i]);
    }
