...
// Main test function
void test_ping_to_ping_correlationRoee() {
    int NumDetect = 13;
    int ping_ind = 1;
    int Tping = 1;
    ...
    ping_to_ping_correlationRoee(TracksMat, TracksVecMat, TracksP, TracksX, TracksMissMat, TracksDataBinMat, TracksDataMat,
                                 MergedRng, MergedTeta, MergedYc, NumDetect, ping_ind, Tping, sigmaTeta, 360, 360,uncorr_plots_list);
    ...
    /*
        ...
        free(TracksX);
        free(TracksMat);
        free(TracksDataBinMat_after);
        free(TracksDataMat_after);
        free(TracksMissMat_after);
        free(TracksP_after);
        free(TracksVecMat_after);
        free(TracksX_after);
        free(TracksMat_after);
    */
};
