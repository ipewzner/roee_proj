...
void clean_tracks(
    ...
    int rows_mat, int cols_mat, int rows_x, int rows_p, int no_of_pings,
    int MaxTracks, int MaxTarget) {
    // Initialize new arrays
    double* TracksNewP = (double*)malloc(MaxTracks * 16 * sizeof(double));                     // 4x4 flattened to 16
    double* TracksNewX = (double*)malloc(4 * MaxTracks * sizeof(double));                      // 4xMaxTracks
    double* TracksNewMat = (double*)malloc(3 * MaxTracks * sizeof(double));                    // 3xMaxTracks
    double* TracksVecNewMat = (double*)malloc(4 * MaxTracks * sizeof(double));                 // 4xMaxTracks
    double* TracksNewMissMat = (double*)malloc(5 * MaxTarget * no_of_pings * sizeof(double));  // 5xMaxTargetxno_of_pings
    double* TracksNewDataBinMat = (double*)calloc(MaxTarget * MaxTarget, sizeof(double));      // binary matrix
    double* TracksNewDataMat = (double*)malloc(MaxTarget * MaxTarget * sizeof(double));        // 999 initialized

    // Initialize new arrays with default values
    for (int i = 0; i < MaxTracks * 16; i++) {
        TracksNewP[i] = 999.0;
    }
    for (int i = 0; i < 4 * MaxTracks; i++) {
        TracksNewX[i] = 999.0;
        TracksVecNewMat[i] = 999.0;
    }
    for (int i = 0; i < 3 * MaxTracks; i++) {
        TracksNewMat[i] = 999.0;
    }
    for (int i = 0; i < 5 * MaxTarget * no_of_pings; i++) {
        TracksNewMissMat[i] = 999.0;
    }
    for (int i = 0; i < MaxTarget * MaxTarget; i++) {
    ...
    for (int i = 0; i < undeleted_tracks_count; i++) {
        ...
        // Copy TracksMat
        for (int j = 0; j < 3; j++) {
            TracksNewMat[j * MaxTracks + i] = TracksMat[j * cols_mat + idx];
        }

        // Copy TracksX
        for (int j = 0; j < 4; j++) {
            TracksNewX[j * MaxTracks + i] = TracksX[j * cols_mat + idx];
        }
        ...
        // Copy TracksVecMat
        for (int j = 0; j < 4; j++) {
            TracksVecNewMat[j * MaxTracks + i] = TracksVecMat[j * cols_mat + idx];
        }

        // Copy TracksMissMat
        for (int j = 0; j < 5; j++) {
            for (int k = 0; k < no_of_pings; k++) {
                TracksNewMissMat[j * MaxTarget * no_of_pings + i * no_of_pings + k] = TracksMissMat[j * MaxTarget * no_of_pings + idx * no_of_pings + k];
            }
        }

        // Copy TracksDataBinMat
        for (int j = 0; j < MaxTarget; j++) {
            TracksNewDataBinMat[i * MaxTarget + j] = TracksDataBinMat[idx * MaxTarget + j];
        }

        // Copy TracksDataMat
        for (int j = 0; j < MaxTarget; j++) {
            TracksNewDataMat[i * MaxTarget + j] = TracksDataMat[idx * MaxTarget + j];
        }
    }

    // Step 3: Replace old structures with new structures
    memcpy(TracksMat, TracksNewMat, 3 * MaxTracks * sizeof(double));
    memcpy(TracksX, TracksNewX, 4 * MaxTracks * sizeof(double));
    memcpy(TracksP, TracksNewP, MaxTracks * 16 * sizeof(double));
    memcpy(TracksVecMat, TracksVecNewMat, 4 * MaxTracks * sizeof(double));
    memcpy(TracksMissMat, TracksNewMissMat, 5 * MaxTarget * no_of_pings * sizeof(double));
    memcpy(TracksDataBinMat, TracksNewDataBinMat, MaxTarget * MaxTarget * sizeof(double));
    memcpy(TracksDataMat, TracksNewDataMat, MaxTarget * MaxTarget * sizeof(double));
...
void initialize_tracks(double* TracksMat, double* TracksX, double* TracksVecMat, double* MergedYc, double* MergedRng, double* MergedTeta, int NumDetect, double ping_number) {
    ...
    for (int i = 0; i < NumDetect; i++) {
        ...
        TracksMat[i + NumDetect] = 0;      // TracksMat(2, 1: NumDetect) = zeros(1, NumDetect);
        TracksMat[i + 2 * NumDetect] = 0;  // TracksMat(3, 1: NumDetect) = zeros(1, NumDetect);
    }

    // Initialize TracksX (flattened)
    for (int i = 0; i < NumDetect; i++) {
        TracksX[i] = MergedYc[i];                          // TracksX(1, 1: NumDetect) = MergedYc(1, :);
        TracksX[i + NumDetect] = MergedYc[i + NumDetect];  // TracksX(2, 1: NumDetect) = MergedYc(2, :);
        TracksX[i + 2 * NumDetect] = 0;                    // TracksX(3, 1: NumDetect) = zeros(1, NumDetect);
        TracksX[i + 3 * NumDetect] = 0;                    // TracksX(4, 1: NumDetect) = zeros(1, NumDetect);
    }

    // Initialize TracksVecMat (flattened)
    for (int i = 0; i < NumDetect; i++) {
        TracksVecMat[i] = 1;                              // TracksVecMat(1, 1: NumDetect) = ones(1, NumDetect);
        TracksVecMat[i + NumDetect] = MergedRng[i];       // TracksVecMat(2, 1: NumDetect) = MergedRng;
        TracksVecMat[i + 2 * NumDetect] = MergedTeta[i];  // TracksVecMat(3, 1: NumDetect) = MergedTeta;
        TracksVecMat[i + 3 * NumDetect] = ping_number;    // TracksVecMat(4, 1: NumDetect) = ping_number * ones(1, NumDetect);
    }
}
...
void AnalyzeTracks(double* TracksMat, double* TracksVecMat, double* TracksP, double* TracksX, double* TracksMissMat, double* TracksDataBinMat, double* TracksDataMat,
                   double* MergedRng, double* MergedTeta, double* MergedYc, int ping_number, double Tping, double sigmaTeta, double Win_dlt, double Dlt_th,
                   int MaxTracks, int CurrentTargetInd, int numDetect, int MaxTarget, int no_of_pings) {
    ...
    if (ping_number > 0) {
        ...
        clean_tracks(TracksMat, TracksX, TracksP, TracksVecMat, TracksMissMat, TracksDataBinMat, TracksDataMat,
                     3, 360, 4, 4, no_of_pings, MaxTracks, MaxTarget);
